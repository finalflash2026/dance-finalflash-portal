...
- **設定は同じボタンで閉じられる**(v1.14.2)。開いている間はアイコンを × に変え、押すと直前に見ていた画面へ戻る。以前は下のタブを押すしかなく、「全体カレンダーを見ていたのに、閉じたらマイカレンダーにいた」が起きていた。
  - 直前の画面は**ヘッダのボタン自身が覚える**。置き場所がレイアウトで、タブ間の移動でも設定への移動でも作り直されないため。
