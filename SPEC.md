...
| お知らせ | ナンバー関連のみ受信(`schedule_updated` と公式練の `attendance_updated` は配信対象外) |
...
### 5.2 マイグレーションSQL(全文)
`supabase/migrations/0001_init.sql` として保存し、Supabase SQL Editor またはCLIで適用する。
...
- 予定が存在する日にドット表示(タブ①=公開slotsのある日 / タブ②=所属ナンバーの予定がある日 / タブ③=自分の予定がある日)。
...
**空き申請フロー(v1.1: 自由時間帯)**
1. 空きブロックをタップ → 申請シートを表示。対象部屋の当日タイムバー(9〜22時)を描画し、不可・公式練・申請済は塞がった表示、空き範囲内で開始/終了をドラッグまたはプルダウン選択(**15分刻み**)。
...
**Step2: コマ割りエディタ**
1. 月を選ぶと予約枠一覧(日付順)。各予約枠を展開すると、その枠内のコマ編集UI。
2. コマ追加: 開始/終了時刻(プリセットボタン: 90分刻み・110分刻み・手入力)、状態(公式練/空き/使用不可)、ジャンル、対象期(複数選択・未選択=全期)。
3. バリデーション: コマは親予約枠の時間内に収まる。同一予約枠内のコマ同士は時間が重ならない。
4. 予約枠のうちコマ未設定の時間帯はUI上「未割当」とグレー表示(DB行は作らない)。
...
**カレンダービュー(タブのメイン)**: ミニカレンダー(所属ナンバーの予定がある日にドット)→ 日付タップで縦タイムライン(行=時間軸のみ、列なし。Appleカレンダー様式)。各予定ブロックに「ナンバー名 @場所」を表示。ナンバーごとに色を自動割当。
...
2. **お知らせカード**: `notifications` を新しい順に表示(未読は強調、タップで既読化=`read_at`更新)。種類は
   - `number_added`: 他者が作ったナンバーに自分が追加された(トリガで自動生成)
   - `schedule_updated`: 折衝が月を公開/公開済コマを変更した(公開APIが全ユーザーへ一括insert。文言例「8月の練習日程が更新されました」)
   - `attendance_updated`: 公式練/ナンバー練の出欠が更新された(トリガで自動生成。§6.4.2)
3. **絞り込みチップ**(横スクロール): `すべて / 公式練(=公式練+自分の申請) / {所属ナンバー名}…`。選択状態はローカル保存。
4. **ミニカレンダー**(チップの絞り込みを反映したドット)→ 日付タップで縦タイムライン(行=時間軸のみ、列なし)。予定ブロックのタップで出欠管理窓(§6.4.2)。
...
**操作(下部): 自分の出欠登録**
- ボタン3つ: 「欠席」「遅刻」「早退」。
- 「遅刻」「早退」を押すと、そのボタン横に時刻入力欄が展開(15分刻みプルダウン+直接入力可)。例: 15時に帰る → 「早退」+「15:00」。
- 「欠席」は時刻不要で即登録(確認ダイアログあり)。
- 登録済みの自分の状態は選び直しで上書き、「出席に戻す」で取消(行削除)。出席の人は何も操作しなくてよい。
- 登録/変更のたびに、対象の参加者(本人以外)のお知らせに通知が届く(文言例「22BREAKせいあが8/6のBREAK公式練を早退(15:00)にしました」)。
...
### 13.3 バックアップ
- Supabase無料枠は自動バックアップ無し。**GitHub Actionsで日次 `pg_dump` を取得しプライベートリポジトリ/外部ストレージへ保存**するワークフローを用意する(フェーズ4で可)。
...
**Phase 2: 空き申請**
- [ ] 空きコマ内の自由時間帯(15分刻み)で申請でき、タブ①に申請者と時間帯が表示される
...
- **claims競合**: 同一slotの重複時間帯へ並行insertし、排他制約で片方のみ成功すること。非重複時間帯なら両方成功すること。範囲がコマ外/15分刻み外の申請が弾かれること。
