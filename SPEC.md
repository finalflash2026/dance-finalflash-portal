...
本節 = 0001 + 0002 + 0003 + 0004 を適用した状態。
...
3. バリデーション: コマは親予約枠の時間内に収まる。同一予約枠内のコマ同士は時間が重ならない。**重なりは`slots`の排他制約でDB側でも禁止する**(§5.2)。
4. 予約枠のうちコマ未設定の時間帯はUI上「未割当」とグレー表示(DB行は作らない)。
5. **予約枠の取消(v1.8.1)**: 各予約枠に取消ボタンを置く。取込確定後に読み取りミスに気付いたときの唯一の訂正手段。
...
4. **ミニカレンダー**(チップの絞り込みを反映)。**各日のマスに予定ラベルを表示する**(v1.7で変更。ドットではなく`18:00 PUNKING`のように時刻＋名称を並べる。マスに入りきらない場合は末尾に`+N件`)。→ 日付タップで縦タイムライン(行=時間軸のみ、列なし)。予定ブロックのタップで出欠管理窓(§6.4.2)。
