...
| v1.27.1 | 使ってみて出た小さな2件(§12)。①**連絡の窓の下端をホームバーのぶん空けた**。ホーム画面から開くと画面の隅まで自前で描く設定になっていて、下に固定した「書き込む」「閉じる」がホームバーの下に潜っていた。この窓はボタンを固定しているのでスクロールでも逃げられない。②**設定の × が「1つ前」ではなく「設定に入る前」へ戻る**ように。設定 → 管理者 → 設定 と辿ってから閉じると、閉じたはずが管理者画面に出ていた。画面はタブ → 設定 → 管理者/折衝 の順に積み上がるので、設定より上はまとめて畳む |
...
- **下から出る窓は、下端をホームバーのぶん空ける**(v1.27.1)。`viewport-fit=cover` で画面の隅まで自前で描いているため、一番下に置いたものは iPhone のホームバーの下に潜る。とくに**ボタンを下に固定した窓**(連絡欄 §6.1.3)は、スクロールでも逃げられないまま押しにくくなる。下の余白に `env(safe-area-inset-bottom)` を足す(タブバーと同じ考え方)。
