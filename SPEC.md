...
- **文言は1行で完結させる**(v1.15.1)。ロック画面では見出ししか読まれないことが多く、2行に分けると肝心の「誰が」「どこを」が畳まれて見えなくなる。
  - 施錠/開錠 … `{ユーザーID}が{場所名}を開けました` / `…を閉めました`
  - 部室の鍵 … `{ユーザーID}が鍵を持っています`
  - 日程公開 … `新しい日程が公開されました`(月と件数はアプリ内のお知らせ側に出す。端末の通知は開くきっかけになれば足りる)
