...
| v1.27.1 | 使ってみて出た小さな2件(§12)。①**下から出る窓の下端をホームバーのぶん空けた**(7か所すべて)。ホーム画面から開くと画面の隅まで自前で描く設定になっていて、一番下のボタンがホームバーの下に潜っていた。最初に見つかったのは連絡欄(ボタンを下に固定していてスクロールでも逃げられない)だが、原因は窓に共通なので `.safe-bottom` を1つ用意して全部に付けた。②**設定の × が「1つ前」ではなく「設定に入る前」へ戻る**ように。設定 → 管理者 → 設定 と辿ってから閉じると、閉じたはずが管理者画面に出ていた。画面はタブ → 設定 → 管理者/折衝 の順に積み上がるので、設定より上はまとめて畳む |
| v1.27 | **掲示板に連絡欄を追加**(§6.1.3 / §6.6 / §8.12)。施錠状況と部室の鍵の2つのボードに、短いやり取りを足せるようにした。○×だけでは「控室136は開けていないが、鍵はリハ室の人が持っている」が表せない。**書き換えではなく足していく**(この種の話は「持ってます」→「渡しました」と続く)。消せるのは書いた本人だけ。ボードには1行だけ置き、読み書きは下から出る窓で行う(直接展開するとカレンダーが画面外へ押し出される)。書き込みは**サーバー経由**で、通知は別カテゴリ |
| v1.26 | プッシュ通知を**1行目=誰が / 2行目=何をしたか**に(§6.6)。ユーザーIDと場所名がどちらも長く、1行に収めると iPhone のロック画面で肝心の「開けました/閉めました」まで表示されずに切れていた |
| v1.25 | スタ練に**対象期と編集**を追加(§6.3.1)。①公式練と同じく期で絞れるようにした(既定は全期)。②登録済みのスタ練を直せるようにした。削除しか無く、時間を間違えたら消して作り直すしかなかった。**スタ練は折衝がコマを割り振っていないだけで中身は公式練と同じ**、という整理に合わせている |
| v1.24 | **スタ練にも出欠を付ける**(§6.3.1 / §6.4.2)。当初は「誰が来るか決めない性質」として外したが、「行けない」「遅れる」を先に共有したいのは主催が公式かどうかと関係が無かった。参加者を集める条件は公式練と共用する(2か所に持つと片方だけ直す事故が起きる) |
| v1.23 | **スタ練を追加**(§6.3.1)と**タブ②の再構成**(§6.3)。①ジャンルごとの自主練を、折衝を通さずに登録できるようにした。**メンバーを選ばず、そのジャンルを取っている人全員に自動で共有される**。立てられるのは自分の1ジャンだけ。マイカレンダーには公式練と同じ色で出し、絞り込みも公式練と同じ扱いにする。②タブ②の並びを 現在進行中のナンバー → ナンバーをつくる → スタ練を設定する に(ナンバーだけのカレンダーは要らなかった) |
| v1.22 | **空き申請の取消を本人だけに**(§6.1)。折衝・管理者にも取消ボタンが出ており、実際に他人の申請を取り消せてしまうと報告があった。RLS (`del_claims`) と画面の両方で本人に絞る。折衝が申請を直接消す必要は無い(コマごと消せば cascade で消える) |
| v1.21 | **ナンバー練の日跨ぎ対応**(§6.3)と、**日程一覧からの出欠連絡**(§6.4.2)。①`23:00〜翌06:00` のような夜通しの練習を登録できるようにした。**終了が開始以前なら翌日まで**と読む約束にし、判断を `lib/day-span.ts` の1か所に閉じ込めた。カレンダーでは2日ぶんに割って両日に出す。ics は終了を「終わる日」で書く。②ナンバー詳細の日程一覧の行をタップすると出欠管理窓が開くようにした。これまではカレンダー経由でしか開けなかった |
| v1.20.1 | 公開リポジトリから**実在の登録者のユーザーIDを削除**(§3.1)。仕様書とコードの例に実在の22期の方のIDを使っていた。公開している以上、本人の所属・期・ジャンルが誰にでも読める状態だった。例は `22BREAKたろう` のような架空の名前に統一する |
| v1.20 | **練習場所まわりの改修**(§4.2 / §6.2 Step1 / §8.11)。①**柔道場(体育館)と会議室1(南大沢市民センター)を追加**。②**練習場所を後から増やせるようにした**。取込CSVに知らない場所が出てきたら、折衝係が Step1 の画面からその場で登録して先へ進める。以前は使える場所が増えるたびにコードを直してデプロイする必要があった。これに伴い、画面が参照する部屋の一覧を**固定の定数からDBへ移し**、Context で配る形にした(11ファイル)。所在(section)の一覧も固定をやめ、部屋から組み立てる |
| v1.19 | **新規登録画面の改修2件**(§3.2 / §8.1)。①**マイパスワードを2回入力させる**。メールを持たない設計なので、打ち間違えたまま登録すると本人にも気づけず、管理者に再設定を頼むしかなくなる。画面とAPIの両方で突き合わせる。②**2ジャン・3ジャンを登録時に選べる**ようにした。登録直後からマイカレンダーに出したいのに、設定画面まで行かないと選べなかった。あとから設定画面で変えられる点は変わらない |
| v1.18.1 | 設定画面の見直し(§6.4.1 / §12)。①各項目を**枠で囲む**。見出しと余白だけでは、どこからどこまでが1項目か分からなかった。②**「ロール」→「権限」「昇格」→「権限を追加する」**に言い換え(IT用語で意味が伝わっていなかった)。③`admin` の呼び名を**「3役(管理者)」**に(サークル内の呼び方に合わせる)。④権限の欄を下げ、**設定 → 権限 → ログアウト**の並びにした。日常的に使うのは通知やテーマで、権限は一度きりの操作のため |
| v1.18 | 起動の速さ(§13.1 / §6.6)。実測の結果、遅さの主因は **iOS がホーム画面アプリのキャッシュを捨てるため JavaScript 805KB を取り直していたこと**だった(Supabase のコールドスタートではない)。Service Worker に **`/_next/static/` だけ**を持たせる。ファイル名にハッシュが入るので「古い版が出続ける」事故が原理的に起こらない範囲に限っている。ページのデータは従来どおり持たない |
| v1.17 | **既定のテーマをダークに変更**(§12)。判定を「`dark` か」から「`light` か」に反転させ、未設定・壊れた値はダーク扱いにする。**自分でライトを選んだ人の設定は変わらない**(`light` が保存されているため)。あわせて静的な `theme-color` とマニフェストの `background_color` をダークの地色に揃えた。`theme-color` を静的に出している限り、v1.13.2 の「ライトなのに上端が黒い」は再発しない |
| v1.16.1 | 運用体制の変更(アプリの動作に変更なし)。**日次バックアップを別リポジトリ `dance-finalflash-ops` へ分離**(§13.2 / §13.3)。GitHub Actions の成果物は**リポジトリの読み取り権限があれば誰でもダウンロードでき**、DBの完全なダンプは service role key と同等に重い。本体リポジトリをコードの閲覧用に開放するにあたり、「コードを見せてよい相手」と「データを渡してよい相手」を別々に判断できるようにした |
| v1.16 | **機能追加1件**。**引っぱって更新**(§12)。ホーム画面から開くとブラウザの下スワイプ再読み込みが無くなるため、自前で用意した。standalone のときだけ効かせる(タブでは二重に読み込むため)。`router.refresh()` に加えて、自分で Supabase を引いている施錠ボード・部室の鍵ボードにも合図を送る(`lib/refresh-signal.ts`)|
| v1.14.2 | テスト運用の指摘による調整4件。①下部タブバーの島を **300px** に(幅いっぱいは長すぎた。§12)。②**設定ボタンで設定を閉じられる**ようにし、開いている間はアイコンを × に変えて直前の画面へ戻す(§12)。以前は下のタブを押すしかなく、見ていた画面に戻れなかった。③**タブ②を開いた時点で所属ナンバーと新規作成を出す**(§6.3)。「ナンバー管理」ボタンの先に置いていたのを取り込み、`/numbers` は `/number-cal` へ転送に変更。④部室の鍵のボタンを「**所持中に変更**」にし、すりガラス + 角丸を大きく(§6.1.2)。押すと何が起きるかを書く文言に改めた |
| v1.15.1 | 通知の文言を1行で完結する形に変更(§6.6)。ロック画面では見出ししか読まれないことが多く、「誰が」を本文へ回すと畳まれて見えなくなるため。`22BREAKたろうがスタジオ101(7号館)を開けました` / `22BREAKたろうが鍵を持っています` / `新しい日程が公開されました` |
...
| ユーザーID(username) | `{期}{1ジャンコード}{名前}` 例: `22BREAKたろう`(**例に実在の登録者を使わないこと**。このリポジトリは公開されており、実在のIDを書くと本人の所属・期・ジャンルが誰にでも読める。v1.20.1) |
...
- 入力項目: 期(数値)、1ジャン(9択)、名前(ひらがな/カタカナ/漢字可)、**2ジャン・3ジャン(任意)**、マイパスワード(8文字以上)、**マイパスワード(確認)**、**サークル生合言葉**。
- **マイパスワードは2回入力させる**(v1.19)。このサイトは**メールを持たない**ので、打ち間違えたまま登録すると本人にも気づけず、管理者に再設定を頼むしかなくなる。**画面とAPIの両方で突き合わせる**(APIは直接叩けるため)。
- **2ジャン・3ジャンも登録時に選べる**(v1.19)。登録直後からマイカレンダーと購読URLに含めたいのに、設定画面まで行かないと選べなかった。**あとから設定画面で変えられる点は変わらない**(§6.4.1)。
  - 制約は設定画面と同じ: 1ジャンと同じものは選べない / 2ジャンと3ジャンに同じものは選べない / 未設定でよい。
  - **1ジャンを選び直したら、同じジャンルを選んでいたサブジャンルは自動で外す。** 残したままだとサーバーに弾かれるが、画面上は矛盾して見えないため「なぜ登録できないのか」が分からなくなる。
...
| アリーナ | 剣道場(体育館) |
| アリーナ | 柔道場(体育館) |
| 南大沢市民センター | 会議室1(南大沢市民センター) |

(初期12件 + v1.20 で2件。①カレンダーの列はこの順で section ごとにグループ表示する)

**部屋は後から増やせる**(v1.20)。
- **正はDBの `rooms` テーブル**。コード側の固定リスト(`lib/constants.ts` の `ROOMS`)は **Supabase 未設定でも画面が出るようにするための控え**でしかなく、増えたぶんは入っていない。画面では `lib/rooms.tsx` の `useRooms()` / `useRoomById()` / `useRoomSections()` を使うこと。
- 取得は `(app)/layout.tsx` が1回だけ行い、Context で配る。使う場所が日別グリッド・申請シート・施錠ボード・折衝の各画面と広く階層も深いため、props で通すと途中のサーバーコンポーネントまで巻き込むことになる。**profile の取得と並列**に走らせているので往復は増えていない(§13.1)。
- **section(所在)の一覧も固定で持たない。** 南大沢市民センターのように所在ごと増えるため、部屋の並び順に現れた順で組み立てる。
- 追加は折衝係が取込画面から行う(§6.2 Step1)。`id` と `sort_order` は**DBの連番に決めさせる**(手で番号を選ばせても衝突や飛び番を生むだけ)。
...
- `0009_rooms_dynamic.sql` … v1.20 の差分(練習場所の追加・id の自動採番。§4.2)
- `0010_number_events_overnight.sql` … v1.21 の差分(ナンバー練の日跨ぎ。§6.3)
- `0011_claims_owner_only.sql` … v1.22 の差分(空き申請の取消を本人だけに。§6.1)

本節 = 0001 〜 0011 を適用した状態。
...
create table public.reservations (
  ...
  created_at timestamptz not null default now(),
  check (start_time <> end_time)   -- 終了が開始以前なら翌日まで (v1.21 / §6.3)
);
...
-- **取消は本人だけ** (v1.22)。折衝が申請を直接消す必要は無く、
-- コマごと消せば slot_id の cascade で一緒に消える
...
アプリ本体は下部固定タブバーで3画面を切り替える(代表デザイン準拠)。**並びは左から**:
- **マイカレンダー … §6.4(`/`。既定で最初に開く画面)**
- 全体カレンダー(ジャンル練+空き申請) … §6.1(`/overview`)
- ナンバーカレンダー … §6.3(`/number-cal`)

> **v1.14で並びとURLを変更した。** 毎日開くのはマイカレンダーで、以前は起動のたびに全体カレンダーから1タップ移動していた。既定の画面を `/` そのものにしてあるので、ホーム画面のアイコンから開くと転送を挟まず直接表示される。旧URL `/me` は `/` へ308で転送し、既存のブックマークを死なせない。
>
> 本書では歴史的に「タブ①=全体 / タブ②=ナンバー / タブ③=マイ」と番号で参照している箇所が残っているが、**番号は画面の並び順ではなく識別子**として読むこと。

**OB/OGの表示(v1.6)**: `role='ob'` のユーザーには全体カレンダーを表示せず、**2タブ構成**とする。初期表示はマイカレンダー。`/overview` へのURL直打ちは `/` へリダイレクトする。
...
5. 自分の申請はタブ③と購読icsに自動反映。**取消は本人だけ**(v1.22)。折衝・管理者にも許していたが、他人の申請を取り消せてしまうと報告があった。申請は「その人がその時間に練習する」という本人の意思表示で、**消えたことに本人が気づく手段が無い**。折衝がまとめて無効にしたい場面はコマごと消すときで、そちらは `claims.slot_id` の cascade が面倒を見るため、直接消す権限は要らない。RLS (`del_claims`) と画面の両方で本人だけにする。
...
- 各行に最終更新情報を小さく添える(例: `14:32 22BREAKたろう`)。誰が開け閉めしたかが分かることで、鍵の所在を追える。
...
**更新の鮮度**
- カードは**15秒ごとに自動再取得**し(v1.14で60秒から短縮)、画面上部に「最終取得 HH:MM」を表示。手動更新にも対応する。鍵を開けた直後に他の人の画面へ反映されないと、行ってから「開いていない」と勘違いされるため。
- **画面が隠れている間はポーリングを止め、戻った瞬間に1回取り直す**。誰も見ていない間に叩き続けても通信とバッテリーの無駄で、逆に「ポケットから出した直後」は古い値が一番まずい。実装は `lib/use-live-refresh.ts` に集約し、部室の鍵ボード(§6.1.2)と共用する。
- **カードが表示されるたびに、まず1回取り直す**(v1.14.1)。ルーターキャッシュ(§13.1)が効いている間に戻ると、サーバーに問い合わせずに前回のRSCペイロードが再利用され、**最大30秒前の初期値**からポーリングが始まる。合わせると最大45秒古い鍵の状態が出たままになり、実際に「戻ったら開錠の連絡が消えていた」として報告された。カードは自前で Supabase を引くので、表示のたびに1回引けばキャッシュの齢に関係なく現在の値になる。**ページ全体を取り直す(`router.refresh()`)より軽く、速さのためのキャッシュを保ったまま鮮度だけ戻せる。**
...
**表示**
- 現在の所持者の username と、いつから持っているか(例: `22BREAKたろう / 8/20 18:32 から`)。
- 記録が1件も無ければ「まだ登録されていません」と出す。
- 直近の受け渡し履歴を折りたたみで数件添える。行方が分からなくなったとき、最後に持っていた人からたどれるようにするため。
- 更新の鮮度は施錠ボードと同じ扱い(**15秒ごと・非表示中は停止・復帰時に即取得**。v1.14。§6.1.1)。

**操作**
- 「**所持中に変更**」ボタン。押すと自分が所持者になり、全員の画面に反映される。所持者が自分のときは「自分が所持中」と出して押せなくする。
  - 文言は**今の状態ではなく、押すと何が起きるかを書く**(v1.14.2 で「私が持っています」から変更)。事実の申告に読めると、既に自分が持っている人が押してよいのか迷う。
  - 見た目はタブバーと同じ**すりガラス**。枠線だけより「押せるもの」だとはっきり分かる。角は大きめに丸めてタブバーのピルと揃える。
...
#### 6.1.3 掲示板の連絡欄(v1.27追加)

施錠状況ボード(§6.1.1)と部室の鍵ボード(§6.1.2)に、短いやり取りを足せる欄。

**なぜ要るか**: ○×だけでは表せない状況がある。
> 「控室136は開けていないが、鍵はリハーサル室にいる人が持っている」

**書き換えではなく足していく。** この種の話は「持ってます」→「取りに行きます」→「渡しました」と続くのが自然で、1行を上書きする形だと途中が消える。部室の鍵の受け渡しと同じ考え方。

- **消せるのは書いた本人だけ。** 他人の発言を消せると、何が話されていたのか誰にも分からなくなる。更新のポリシーは置かない(言った内容が後から変わらない)。
- 施錠状況の連絡は**日付に紐づく**(ボードが日付でリセットされるため)。部室の鍵の連絡は日をまたいで続く。
- 1件300文字まで。長文は掲示板ではなくLINEでやってもらう。
- 書き込むと**現役全員に通知**が飛ぶ(1行目=ユーザーID / 2行目=本文)。**通知は鍵の開閉とは別のカテゴリ**にして個別に切れるようにする — 連絡は数が多くなりうるので、まとめると「うるさいから鍵の通知ごと切る」が起きる(§6.6)。
- 書き込みは**サーバー経由**(`POST /api/board-messages`)。全員に通知が飛ぶ操作なので、ブラウザから直接 insert させると**書いていない内容を届けられてしまう**(§6.6 と同じ理由)。削除は通知を伴わないので RLS 経由で直接消してよい。

**画面**
- ボードの中には**1行だけ**置く(件数 + 最新の1件)。全体カレンダーには施錠ボード・部室の鍵・ミニカレンダー・日別ビューが縦に並んでおり、ここに会話を直接展開すると**カレンダーが画面外へ押し出される**。
- 読み書きは下から出る窓で行う(申請シート・出欠管理と同じ形)。
- 更新はボードと同じ15秒間隔(§6.1.1)。
...
**タブを開いた時点で管理まで出す**(v1.14.2)。上から順に:

1. **現在進行中のナンバー** … 自分が所属するナンバーの一覧(メンバー数・主催かどうか)。タップで詳細(`/numbers/[id]`)へ
2. **ナンバーを作る** … 名前を入れて作成
3. **ナンバーの予定** … ミニカレンダー → 日別タイムライン

以前はカレンダーだけを出し、一覧と作成は「ナンバー管理」ボタンの先(`/numbers`)に置いていた。**タブ②で一番よくやるのは所属の確認と日程の登録**で、毎回1タップ挟んでいた。`/numbers` は `/number-cal` へ転送するだけにして、同じ内容が2か所に出ないようにする。

**タブ②の構成(v1.23)**

上から **所属ナンバー → ナンバーをつくる → スタ練を設定する**。

**カレンダーは置かない。** 予定を見る場所はマイカレンダーに一本化した — ナンバー練もスタ練も公式練も、その人にとっては同じ「自分の予定」で、タブごとに別のカレンダーを見に行く理由が無い。タブ②は**予定を作る側**の画面という位置づけ。

**日をまたぐ予定(v1.21)**

ナンバー練には `23:00〜翌06:00` のような夜通しの練習がある。**終了が開始以前なら「翌日まで」と読む**(営業時間の書き方と同じ約束)。

- 判断は `lib/day-span.ts` の1か所に閉じ込める。**日付をまたぐかを見たい場所は必ずここを通す。** 散らばると、片方の画面だけ翌日ぶんを描き忘れる・ics の終了だけ前日のままになる、といったずれ方をする。
- カレンダーでは**2日ぶんに割って両日に出す**(1日目=開始〜24:00 / 2日目=00:00〜終了)。縦のタイムラインは1日ぶんの高さしか持たないので、割らずに描くと1日目が画面外へはみ出し、2日目には何も出ない。
- 断片には「翌06:00まで」「前日23:00から」を添える。これが無いと、翌朝の画面に 00:00 から始まる予定だけが現れて、いつからの続きか分からない。
- **ics は終了を「終わる日」で書く。** 同じ日付で書くと終了が開始より前になり、カレンダーアプリ側で予定が消えるか長さ0になる。
- 月ぶんの取得は**前日から**引く。前日から跨いできた予定は `date` が月の外にあり、そのままでは1日に何も出ない。
- **開始と終了が同じ時刻は禁止**(DBの CHECK)。0分なのか24時間なのか決めようがない。
- 入力欄では、跨ぐ入力になった時点で画面に知らせる。打ち間違いか意図どおりか、本人にも判断できないため。

**カレンダービュー**: ミニカレンダー(所属ナンバーの予定がある日にドット)→ 日付タップで縦タイムライン(行=時間軸のみ、列なし。Appleカレンダー様式)。**時間軸は既定 09:00〜22:00 だが、そこに収まらない予定がある日は 00:00〜24:00 の全時間帯にする**(v1.11。深夜練が端で切れて見えなくなるのを防ぐ)。各予定ブロックに「ナンバー名 @場所」を表示。ナンバーごとに色を自動割当。
- **一覧・検索ページは作らない**。並ぶのは自分が所属するナンバーだけ。非メンバーには存在自体不可視(RLSで強制)。
...
公式練とは別に、**ジャンルの有志がスタジオを取って行う練習**。タブ②の一番下「{ジャンル}スタ練を設定する」から登録する。

**ナンバーとの違い**
- **メンバーを選ばない。** 登録すると、そのジャンル(1/2/3ジャンのいずれか)を取っている人**全員のマイカレンダーに自動で出る**。誰が来るかを事前に決める性質のものではなく、名簿を作らせると「入れ忘れた人に伝わらない」が起きる。
- **設定できるのは自分の1ジャンだけ。** 他ジャンルの予定を勝手に立てられると、そのジャンルの人には出所の分からない予定が届く。RLS(`ins_gpractice`)が `genre_id = app_main_genre()` を要求する。
- **直せる・消せるのは登録した本人だけ。** 同じジャンルの他の人にも触らせない(空き申請の取消と同じ考え方。§6.1)。一覧に編集と削除のボタンを出す(v1.25)。

**公式練と同じ扱いにするもの**
- マイカレンダーの表示は「{ジャンル}スタ練」で、**色は公式練と同じジャンル色**。同じジャンルの練習を色で分けると、「BREAKの予定」を探すのに2色を覚えることになる。
- **絞り込みキーも公式練と同じ**。BREAK を押せば「BREAK公式練」と「BREAKスタ練」の両方が出る。利用者から見れば「BREAKの練習」であって、主催が公式かどうかは探すときの関心事ではない。

**公式練と違うもの**
- **対象期を持つ**(v1.25)。公式練と同じ約束で `null` は期を問わない。「1年生だけの基礎練」のような回を立てられるようにするため。既定は全期で、普段は触らずに登録できる。カレンダーへの表示と出欠の書き込みの両方に効く。
- **出欠は付ける**(v1.24)。当初は「誰が来るか決めない性質」として外したが、「行けない」「遅れる」を先に共有したいのは主催が公式かどうかと関係が無かった。**公式練とまったく同じ**(ジャンル + 対象期で絞る)。参加者を集める関数は公式練と共用する — 条件を2か所に持つと、片方だけ直す事故が起きる。
- 日をまたぐ指定は §6.3 と同じ約束(終了が開始以前なら翌日まで)。

**OB には出さない。** 公式練を見られない以上、ジャンル単位の練習にも関わらない(§3.6)。
...
- **ナンバー詳細の日程一覧からも開ける**(v1.21)。行をタップすると同じ窓が出る。これまではカレンダー経由でしか開けず、日程が並んでいる画面から出欠を出せなかった。**主催でなくても押せる** — 出欠は全員が答えるもので、日程の編集権限とは別の話。
...
- 各項目は**枠で囲む**(v1.18.1)。掲示板カード(§6.1.1)と同じ枠にして、画面全体で「囲まれているもの = ひとまとまり」と読めるようにする。共通のクラスは `components/ui.tsx` に置く。
- 並びは **アカウント → サブジャンル → 購読URL → 通知 → 表示テーマ → 権限 → ログアウト**(v1.18.1)。権限は折衝・3役になるときの一度きりの操作なので下げる。ログアウトは設定ではなく操作なので最後。
- **画面に出す言葉に IT 用語を使わない**(v1.18.1)。「ロール」「昇格」は利用者に意味が伝わっていなかった。**「権限」「権限を追加する」**と書く。`admin` の表示は**「3役(管理者)」**(サークル内では「3役」と呼ばれており、「管理者」だけでは自分のことだと気づかれなかった)。
...
- 確認ダイアログに「変更後のログインIDは `23HIPHOPたろう` になります。本人へ必ず連絡してください」と新IDを明示する。
...
- **1行目に「誰が」、2行目に「何をしたか」**(v1.26。v1.15.1 の「1行で完結」から変更)。ユーザーIDと場所名がどちらも長く、1行に収めると iPhone のロック画面で**肝心の「開けました/閉めました」まで表示されずに切れていた**。通知で一番知りたいのは開いたかどうかなので、そこを2行目の先頭側に置く。
  - 施錠/開錠 … 1行目 `{ユーザーID}` / 2行目 `{場所名}を開けました`・`…を閉めました`
  - 部室の鍵 … 1行目 `{ユーザーID}` / 2行目 `部室の鍵を持っています`
  - 日程公開 … `新しい日程が公開されました`(元から短いので1行のまま。月と件数はアプリ内のお知らせ側に出す)
...
- **ページ(HTML・RSC)は絶対にキャッシュしない。** ここを持つと、修正を出したのに一部の端末だけ古い画面のまま、という事故が起きる。**不具合の復旧が届かないのが一番まずい。**
- **`/_next/static/` だけは持つ**(v1.18。§13.1)。ファイル名にビルドのハッシュが入るため、中身が変われば別名になり、古いものが残っていても参照されない。上の事故が**原理的に起こらない**範囲に限って持たせている。
- `fetch` ハンドラは `/_next/static/` の GET 以外で `respondWith` を呼ばない。呼ばなければブラウザが普段どおり取りに行くので、ページも API も挙動が変わらない。
...
| `/` | member + ob | **マイカレンダー(§6.4)。既定で最初に開く画面**(v1.14) |
| `/overview` | member(**OB不可**) | 全体カレンダー(§6.1)。OBは `/` へリダイレクト |
| `/number-cal` | member + ob | **タブ② ナンバー**(§6.3)。所属一覧 + 新規作成 + カレンダー |
| `/me` | member + ob | 旧マイカレンダー。`/` へ**308で転送**(既存のブックマーク救済。v1.14)。**`(calendar)` グループの外**に置く — 中だと `loading.tsx` が先にシェルを流して200になり、ブラウザが新URLを覚えない |
| `/settings` | member + ob | 設定(§6.4.1)。OBはサブジャンル設定を非表示 |
| `/numbers` | member + ob | 旧ナンバー管理。`/number-cal` へ**転送**(v1.14.2。中身をタブ②へ取り込んだ) |
...
- `passwordConfirm` を必ず受け取り、`password` と一致しなければ400(v1.19)。画面側で止めていても API は直接叩けるため。
- `subgenre2Id` / `subgenre3Id`(任意・null可)を受け取り、`profiles` の後に `user_subgenres` へ入れる。**ここで失敗したら auth ユーザーごと消して巻き戻す**(profiles は cascade で一緒に消える)。1ジャンとの重複・2と3の重複は API 側でも弾く。
...
### 8.11 `POST /api/rooms`(v1.20追加)
練習場所を1つ足す(§4.2 / §6.2 Step1)。coordinator 以上。

- `{ name, section, alias? }`。`id` と `sort_order` は**受け取らない**(DBの連番に決めさせる)。
- **service role で書く。** `rooms` はクライアントに insert を許していない — 予約枠・コマ・施錠ボードが参照する土台のデータで、誰でも増やせると取り違えが起きる。代わりにここでロールを検証する。
- 同じ名前が既にあれば**その行を返して使ってもらう**。「登録できません」で止めると、押した人は次に何をすればよいか分からない。
- `alias`(CSVに書かれていた表記)は `room_aliases` に upsert し、次回から自動で解決できるようにする。**別名の登録に失敗しても本体は成功として返す** — 部屋さえできていれば取込は進められる。

### 8.12 `POST /api/board-messages`(v1.27追加)
掲示板の連絡を1件足す(§6.1.3)。現役のみ。

- `{ scope: 'room' | 'club_key', body }`。日付は**サーバーが決める** — 施錠状況(`room`)は当日に紐づけ、部室の鍵(`club_key`)は日付を持たない。クライアントに任せると過去の日付へ書ける。
- **サーバー経由にする理由は鍵の切り替えと同じ**(§6.6)。全員に通知が出る操作なので、ブラウザから直接 insert させると**書いていない内容を全員へ届けられてしまう**。
- 書き込みは**セッションのクライアント**で行い(service role ではない)、RLS の条件をそのまま効かせる。
- 通知はカテゴリ `message`。応答を返してから `after()` で送る。
- **削除にAPIは要らない**(通知を伴わないため)。画面から RLS 経由で消す。
...
**配色とテーマ(v1.13)**
- **ライトモード(白背景)とダークモード(グレー背景)の2種類**を用意し、設定画面から切り替える(§6.4.1)。**既定はダーク**(v1.17。練習は夜が多く、暗い場所で開くことがほとんどのため)。ダークは**真っ黒にはしない**(グレーを地にして、面が上がるほど明るくする)。
  - 判定は「`light` と保存されているか」で行う。`dark` かどうかで見ると、保存値が壊れていたときにライトへ落ちる。**自分でライトを選んだ人の設定は変えない**(その人の localStorage には `light` が入っているため)。
  - CSS の素の `:root` はライトのままにしてある。ダークは `[data-theme="dark"]` が付いたときだけ効くので、既定の切り替えは初期化スクリプトと `readStoredTheme` だけで済む。CSS の役割を入れ替えると全トークンを書き直すことになる。
...
- **`background_color` は既定テーマの地の色と揃える**(v1.17 でダークの `#22272e`)。iOS はこれを起動時のスプラッシュだけでなく**画面上端(ステータスバー周り)の塗り**にも使う。かつて `#000000` にしたところ、ライトモードでも上端が黒く残った(v1.13.2)。**同じ事故を繰り返さない条件は次の `theme-color` を静的に出しておくこと**で、それさえ守れば暗い色でも問題ない。
...
**タブバーと画面の動き(v1.14)**
- 下部タブバーは**画面から浮いた島型**にする(左右に余白・角丸のピル・すりガラス)。背景が透けて動くことで、画面の一部ではなく上に乗っているものだと分かる。`backdrop-filter` は `-webkit-` 付きも併記する(iOS Safari には接頭辞付きしか見ない版がある)。**効かない環境でも文字が読めるよう、ガラスの色は単体で不透明度を確保する。**
- タップ領域は**44px以上**を保つ(浮かせて小さくしない)。浮いたぶん本文の下余白を広げる。
- 島の幅は **300px**(画面がそれより狭ければ幅いっぱい)。各タブが均等に割る。この値は2段階で決めた: 内容なりの幅(約244px)は余白のほうが目立って浮いて見えず、画面いっぱい(約358px)は今度は長すぎた(v1.14.1 → v1.14.2)。
- ヘッダの設定は**三本線のアイコン**(上から順に短くなる)。文字を置くと幅を取り、丸ボタンにも収まらないため。意味は `aria-label="設定"` が持つ。
- **設定は同じボタンで閉じられる**(v1.14.2)。開いている間はアイコンを × に変え、押すと**設定に入る前の画面**へ戻る。以前は下のタブを押すしかなく、「全体カレンダーを見ていたのに、閉じたらマイカレンダーにいた」が起きていた。
  - 戻る先は**「1つ前」ではなく「設定に入る前」**(v1.27.1)。画面は**タブ → 設定 → 管理者/折衝**の順に積み上がるので、1つ前へ戻すと 設定 → 管理者 → 設定 と辿ったときに**閉じたはずが管理者画面に出る**。設定より上に積んだものはまとめて畳む。
  - 入る前の画面は**ヘッダのボタン自身が覚える**。置き場所がレイアウトで、タブ間の移動でも設定への移動でも作り直されないため。`?date=` まで含めて覚え、戻ったときに見ていた日を出す。
  - 設定の上に何も積んでいなければ `router.back()`(履歴を伸ばさない)、積んでいれば行き先を直接指定する。**何回戻ればよいかは数えない** — 端末の戻る操作でも履歴は動くので、数えた回数はすぐ合わなくなる。
  - 覚えが無いとき(URL直打ち・通知から直接開いた・再読み込み)は `router.back()` を**使わない**。サイトの外へ出てしまうので、マイカレンダーへ送る。
  - アイコンは必ず替える。**同じ絵で挙動だけ変わると、押した人には何が起きるか分からない。**
- **横スワイプで隣のタブへ移れる。** 指に追従はさせず、離した時点で移る(追従には3画面ぶんのデータを同時に持つ必要があり、初回表示が重くなる)。
  - **`.h-scroll` と `[data-no-swipe]` の中で始まったタッチは拾わない。** 日別グリッドや取込タイムラインは横スクロールするので、拾うと表が動かせなくなる。モーダルの中も同様(背後の画面が変わってしまう)。
  - 2本指(拡大操作)、縦方向が大きい動き、端のタブでの行き止まり、タブ以外の画面では効かせない。
- **下から出る窓は、下端をホームバーのぶん空ける**(v1.27.1)。`viewport-fit=cover` で画面の隅まで自前で描いているため、一番下に置いたものは iPhone のホームバーの下に潜る。窓は画面の下端に貼り付くので、**最後のボタンが必ずそこに来る**。
  - 実装は `.safe-bottom`(globals.css)を窓に付ける。**余白を上書きせず下に足す**ので、窓ごとの `p-4` / `p-5` はそのまま活き、ホームバーの無い端末では 0 になって従来と変わらない。
  - **窓を1つ足すたびに付ける。** とくにボタンを下に固定した窓(連絡欄 §6.1.3)は、スクロールでも逃げられないまま押しにくくなる。
- 画面遷移は**入場のみ**動かす(移動した向きから滑り込ませる)。モーダルも入場のみ。退場を入れるには閉じる状態を全モーダルに持たせる必要があり、得られる印象に見合わない。
  - アニメーションは**レイアウトに置き、パスが変わったときだけ**再生する。`template.tsx` だと `?date=` が変わるだけでも作り直され、日付を選ぶたびに画面全体が動いてしまう。
- すべて `prefers-reduced-motion: reduce` で無効化する。

**引っぱって更新(v1.16)**
- **ホーム画面から開いたとき(standalone)だけ効かせる。** ブラウザのタブには同じ操作が元から用意されていて、両方が動くと二重に読み込む。standalone にするとその仕組みごと無くなるため、こちらで用意する。
- 拾わない場面: 一番上まで戻っていないとき(普通の上スクロールと区別が付かない)、横に大きく動いたとき(タブの切り替えと競合する)、`[data-no-swipe]` の中(モーダルを開いたまま背後が更新されると、閉じた先が別の画面になっている)。
- **`router.refresh()` と自前カードへの合図の両方を投げる**(`lib/refresh-signal.ts`)。施錠ボードと部室の鍵は自分で Supabase を引いて state に持っているので、サーバー側を描き直しても値が入り直さない(`useState` の初期値は再レンダーでは効かない)。片方だけだと「引っぱったのに鍵の状態が変わらない」になる。
- 完了までの表示は `useTransition` の `isPending` で持つ。指を離した瞬間に戻ると、効いたのかどうか分からない。
- standalone のときだけ `overscroll-behavior-y: contain` を当てる。上端の弾みが指の追従を邪魔するのと、Android の standalone にはブラウザ由来の同じ操作が残っているため。
...
- **クライアント側ルーターキャッシュを30秒保持する**(`next.config.ts` の `experimental.staleTimes`。v1.14)。既定は動的ルートが0秒で、タブを行き来するたびに取り直してスケルトンが出ていた。隣のタブを見て戻る程度の操作が即座に描き直せる。**メモリ上のキャッシュ**なので端末のストレージは使わず、タブを閉じれば消える。
  - 古い値が問題になるものはそれぞれ自衛している: 施錠ボードと部室の鍵は**表示のたび + 15秒ごと**に再取得(§6.1.1/§6.1.2)、空き申請の重複はDBの排他制約が弾く(§6.1)。
  - **キャッシュを縮めて鮮度を取り戻してはいけない。** 30秒を削ると全タブの切替が遅くなり、本当に困るのは鍵の2カードだけ。**リアルタイム性が要るものはカード側で取り直す**のが原則(v1.14.1)。
- **静的ファイルだけ Service Worker に持たせる**(v1.18)。`/_next/static/` には「1年キャッシュしてよい」と指定してあり本来ブラウザが持つが、**iOS はホーム画面アプリのキャッシュを容赦なく捨てる**ため、数日ぶりに開くと 800KB ほどを取り直していた。Cache Storage は消されにくいので、そこへ置く。
  - ファイル名にハッシュが入るので**版管理が要らない**。デプロイのたびに増えるぶんは上限100件で古いものから捨てる(2〜3世代ぶん)。
  - **端末が使う容量は 2〜3MB 程度**。ページのデータは持たないので、ここから増えることはない。
  - 壊れた応答(途中で切れたものなど)は置かない。一度入れると名前が同じ限り出続け、アプリが壊れたままになるため。
  - 登録は**通知を使わない人にも行う**(`components/ServiceWorkerRegistrar.tsx`)。起動直後は取りにいくものが多いので、登録は少し遅らせる。
- **起動の遅さの内訳**(v1.18 実測): サーバーの応答は温まっていれば 320〜500ms、冷えていると 1.4〜2.0秒。配信リージョンは東京(`hnd1`)で、DBクエリも並列化済み。**Supabase のコールドスタートではない。** 残る要因は ①Vercel の関数が冷えること ②JavaScript 805KB の取り直し で、②が上記の対象。
- タブバーの `<Link>` には **`prefetch` を明示する**。動的ルートの既定の先読みは loading 境界までで止まり、中身のRSCペイロードを取らないため(§12のローディング方針と対)。
...
- **バックアップの成果物は、コードと同じ場所に置かない**(v1.16.1 / §13.3)。GitHub Actions の成果物はリポジトリの読み取り権限があれば誰でも取得でき、DBの完全なダンプはデータそのものと同じ重さを持つ。**「コードを見せてよい相手」と「データを渡してよい相手」は別**なので、リポジトリを分けて判断も分ける。
...
### 13.3 バックアップ
- Supabase無料枠は自動バックアップ無し。**GitHub Actionsで日次 `pg_dump` を取得**する。保管先は Actions のアーティファクト(保管90日)。JST 04:00 に実行し、手動実行もできる。
- **バックアップは別リポジトリ(`dance-finalflash-ops`)に置く**(v1.16.1)。理由:
  - **成果物は、リポジトリの読み取り権限があれば誰でもダウンロードできる。** 中身は全ユーザーの情報・合言葉のハッシュ・購読トークンを含む完全なDBで、service role key と同等に重い。
  - 本体リポジトリは**コードを読ませたい相手に Read で開放したい**。同じ場所に置くと「コードを見せる」と「データを渡す」が一体になってしまう。
  - 分けた結果、**人の追加を2つの判断に分けられる**: コードを書いてもらう相手は本体だけ、データを触ってよい相手だけ ops にも入れる。
  - ops 側はコードを触らないため、**60日無操作によるスケジュール停止に本体より先に掛かる**。月1回は有効か確認する。
- 接続文字列は **ops リポジトリの** シークレット `SUPABASE_DB_URL` に置く。**Session pooler(ポート5432)のURLを使う**こと。直結(`db.<ref>.supabase.co`)はIPv6のみで、GitHubのランナーはIPv4のため接続できない。
